            ...
            while True:
                if self.is_computer:
                    ...
                    if ship.place_ship(start_row, start_col, direction, self.board):
                        print(f"El General coloca su {ship.name}.")
        ...
        if self.is_computer:
            while True:
                ...
                if opponent.board[row][col] == ' ':
                    print(f"El General ataca en {row}, {col}. ¡Agua!")
                    self.hits[row][col] = 'A'
                    opponent.board[row][col] = 'A'
                    break
                elif opponent.board[row][col] != 'A':
                    print(f"El General ataca en {row}, {col}. ¡Impacto!")
                    self.hits[row][col] = 'T'
                    for ship in opponent.ships:
                        if (row, col) in ship.positions:
                            if ship.hit():
                                print(f"¡El General ha hundido el {ship.name}!")
                            break
